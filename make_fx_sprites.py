...
배경 처리:
  ...
  - body는 additive dim 파티클 → black_thresh=0 (1px 이상 모두 보존)
"""
...
SPECS = [
    dict(folder="fx_valk_sk1_body",   output="fx_valk_sk1_body_sheet.png",   fw=512, fh=512, cols=8, black_thresh=0),
    dict(folder="fx_valk_sk1_main",   output="fx_valk_sk1_main_sheet.png",   fw=512, fh=512, cols=8, black_thresh=5),
    dict(folder="fx_valk_sk1_weapon", output="fx_valk_sk1_weapon_sheet.png", fw=512, fh=512, cols=8, black_thresh=5),
]


def process_fx_frame(img: Image.Image, black_thresh: int) -> Image.Image:
    """검정 배경 제거 + URP alpha=0 보정 (per-spec threshold)"""
...
def make_sheet(spec: dict) -> bool:
    ...
    for i, fname in enumerate(frame_files):
        ...
        frame = process_fx_frame(frame, thresh)
        if frame.size != (fw, fh):
    ...
    print(f"  OK {spec['output']}  ({cols}x{rows}  {n}frames  {fw}x{fh}px  thresh>{thresh})")
    return True
